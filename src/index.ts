...
const [accuracy, confidence, approxSize, filePath] = process.argv.slice(2);
...
if (!validateConfidence(confidence)) {
  console.error('Confidence (1-δ) must be passed in the second argument\n');
...
if (!validateApproximateSize(approxSize)) {
  console.error('Number of total elements (m) must be passed in the third argument and must be a positive integer\n');
